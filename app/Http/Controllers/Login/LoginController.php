...
class LoginController extends Controller
{
    ...
    public function sendOtp(Request $request): JsonResponse
    {
        ...
        $user = User::where("userLogin", $credentials["username"])->select("userId", "userPassword", "userAccessToken", "userRefreshToken", "userLoginOtp", "userEmail", "userLogin", "userPhoneNumber")->first();
        if ($user) {
            // Validate User
            if (sha1($credentials["password"]) === $user->userPassword) {
                ...
                $smsService->sendTemplateMessage($user->userPhoneNumber, 1, ["otp" => $otp]);
